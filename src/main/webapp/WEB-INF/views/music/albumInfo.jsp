            ...
            <div class="def-block">
                ...
                <div class="products shop clearfix">
                    <div class="clearfix mbs">
                        <div class="grid_6">
                            <img src="api/picture/${selectAlbum.picture }"
                                 style="max-width:330px; min-width:330px; max-height :350px; min-height:350px;">
                        </div>
                        <!-- grid6 -->
                        <div class="grid_6">
                            ...
                            <h3>발매일: <fmt:formatDate pattern="YYYY년 MM월 dd일"
                                                     value="${selectAlbum.releaseAt}"/></h3>
                            <h3>앨범코멘트: </h3>
                            <span style="color: white; font-size:medium;">${selectAlbum.content }</span>
                            <br><br><br><br>
                            <div class="single_variation_wrap" style="text-align:right;">
                                <button class="tbutton medium" onclick="playAll()"><span>전체재생</span>
                                </button>
                                <button class="tbutton medium"><span data-toggle="modal"
                                                                     data-target="#myModal">위시리스트 추가</span>
                                </button>
                            </div>
                        </div><!-- grid6 -->
                    </div><!-- clearfix -->
    ...
    <div class="row row-fluid clearfix mbf">
        <div class="posts">
            <div class="def-block">
                ...
                <button type="button" class="tbutton medium" onclick="ListaddCart()"
                        style="font-size:10px; margin-bottom:7px; margin-left: 7px">
                    <span>선택구매</span>
                </button>
                        ...
                        <form id="playList" action="streamingList" method="post">
                            <div class="bag_table">
                                <table class="shop_table footable tablet footable-loaded"
                                    ...
                                    <thead>
                                    <tr>
                                        <th><input style="margin-right:30px" type="checkbox"
                                                   id="allCheck"></th>
                                        <th></th>
                                    ...
                                    <c:forEach var="music" items="${selectMusicByAlbum}">
                                        <tr data-musicid="${music.id }" class="cart_table_item"
                                            style="text-align: center; font-size: medium;">
                                            <td style="display:none;"><input name="musicIdList"
                                                                             value="${music.id }">
                                            </td>
                                            ...
                                            <c:choose>
                                                <c:when test="${!music.purchase }">
                                                    <td style="vertical-align: middle;"><input
                                                            class="all" style="margin-right:30px"
                                                            name="buyCheck" type="checkbox"
                                                            value="${music.id }"></td>
                                                </c:when>
                                                <c:otherwise>
                                                    <td></td>
                                            ...
                                            <td class="product-name" style="vertical-align:middle;">
                                                <c:choose>
                                                    <c:when test="${!music.purchase }">
                                                        <button type="button" class="tbutton medium"
                                                                onclick="addCart()"
...
<script>
    ...
    function followArtist() {
        $.ajax({
            ...
            contentType: 'application/x-www-form-urlencoded;charset=utf-8',
            traditional: true,
            data: {"artistId": ${selectAlbum.artistId}},
            dataType: "text",
            success: function (data) {
                alert("성공!");
            },
