	...
	<div style="margin: 100px 0px 10px 0px; border: none;">
		...
		<form id="info" name="info" method="post" action="artistRequest"
			...
			<label id="request-label"><div class="sort">작업물 설명란</div> <textarea
					id="artwork" name="artwork" placeholder="내용을 입력해 주세요."></textarea>
			</label><br> <label><div class="sort">작업물 인증</div>
				<input type="file" id="file" name="file"
				accept="image/png, image/jpeg"></label><br>
			...
			<div class="filebox">
				<span>폰번호 입력&emsp;&emsp;<input type="text" id="phone"
					name="phone" placeholder="번호를 입력해주세요(-빼고 입력)"></span> &emsp;
			...
			<br> <span class="point successPhoneChk" style="color: #FFFFE0;">&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;!
				휴대폰 번호 입력 후 인증번호 전송을 클릭 하세요.</span><br>
			<div class="filebox">
				<span>인증번호 입력&emsp;<input type="text" id="phone2"
					name="phone2" placeholder="인증번호를 입력해주세요" disabled required /></span>
				&emsp;
				<p id="phoneChk2" class="doubleChk tbutton" style="">확인</p>
				<br>
			</div>
			<p>
...
<!-- 유효성 검사-->
	...
	function countdown(elementName, minutes, seconds) {
		...
		function updateTimer() {
			...
			if (msLeft < 1000) {
				alert("인증번호가 만료되었습니다.", undefined, undefined, "waring");
				$("" + elementName).remove();
				$("#phone2").attr("disabled", true);
				$("#phoneChk2").css("display", "none");
				$("#phoneChk").removeClass("blind"); // 인증번호 받기 클릭시 숨김처리
			} else {
