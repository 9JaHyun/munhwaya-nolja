...
<div class="container" style="text-align: center;">
    <div class="wrapper">
        <div class="arrow-steps clearfix" style="margin-left:50px; margin-top:6.5em;">
            <div class="step current" id="step1"><span><a
                    style="font-color: white;">아티스트 신청</a></span></div>
        ...
        <h5 style="margin-top:30px; margin-left: 2.4em; width:100%; line-height: 2;"
            id="text1"></h5>
        <!-- 아티스트 승급 재신청 -->
        <br>
        <div>
                ...
                <label>
                    <c:choose>
                        <c:when test="${status eq 'A01' }"> <!-- 승인 -->
                            <a class="tbutton medium" href="mypage.do"
                               style="margin-left: 4em; padding: 5px; margin-top: 2em;">마이페이지로</a>
                        </c:when>

                        <c:when test="${status eq 'A02' }"> <!-- 거절(재신청) -->
                            <form id="info" name="info" method="post" action="artistRequestUpdate"
                                  enctype="multipart/form-data">
                                <div class="sort" style="font-size: 14px; color: white; padding: 0.5em;margin-left: 4em; ">작업물 설명란</div>
                                <textarea id="artwork" name="artwork"
                                          placeholder="내용을 재입력하세요."></textarea>
                                <div class="sort"
                                     style="font-size: 14px; color: white; padding: 0.5em; margin-top:1em; margin-left: 4em;">
                                    작업물 재인증
                                </div>
                                <input type="file" id="files" name="files" multiple="multiple"
                                       accept="image/*" style="margin-left: 10em">
                            </form>
                            <div id="submitBtn" align="right" style="margin-top:2em">
                                <button onclick="submitAgain()"
                                       style="padding: 0.3em 0.5em 0.3em 0.5em; margin: 1em 0.3em 0em 0.3em;"
                                        class="tbutton small">재신청</button>
                                <button style="padding: 0.3em  1em 0.3em 1em; margin: 1em 0.3em 0em 0.3em;"
                                        onclick="location.href='mypage.do'"
                                        class="tbutton small">취소
                                </button>
                            </div>
                        </c:when>
                        ...
                        <c:when test="${status eq 'A03'}"> <!-- 대기(다운로드) -->
                            <label id="request-label">
                                <div class="sort"
                                     style="font-size: 14px; color: white; padding: 0.5em;margin-left: 4em; ">
                                    작업물 설명란
                                </div>
                                <textarea readonly>${status2.getArtwork()}</textarea>
                            </label>
                            <div class="sort"
                                 style="font-size: 14px; color: white; padding: 0.5em; margin-top:3em; margin-left:3em; ">
                                작업물 인증
                            </div>
                            <c:forEach items="${files}" var="file">
                                <span style="margin-left: 1em;">${file.oname}</span>
                                <button type="button"
                                        onclick="location.href='api/attach/${file.id}'"
                                        class="tbutton medium" style="font-size:10px;">
